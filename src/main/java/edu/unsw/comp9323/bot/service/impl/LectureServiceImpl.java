...
public class LectureServiceImpl implements LectureService {
	...
	public String getLectureInfoByWeek(AIWebhookRequest input) {
		Integer week_nb = Integer
				.parseInt(input.getResult().getParameters().get("week-nb").getAsString().replaceAll("\\D+", ""));
		lecture.setWeek(week_nb);
		lecture = lectureDao.getLectureInfoByWeek(lecture);
		return "Week" + week_nb + " on " + lecture.getDate() + " at " + lecture.getLocation();
	}
